...
<div class="modal-full-width">
    <div class="modal-content">
        <div class="modal-header bg-light">
            <h4 class="modal-title" id="myCenterModalLabel">Sửa câu hỏi</h4>
            <button type="button" class="btn-close jquery-btn-cancel" aria-hidden="true"></button>
        </div>
        <div class="modal-body">
            <form class="d-flex justify-content-between flex-wrap" method="POST" action="{{ route('faqs.update', [$faq->id]) }}" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="col-xl-6">
                    <div class="mb-3">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="answer" id="description" cols="30" rows="10" value="{{$faq->answer}}"></textarea>
                        @error('answer')
                        <p style="color: red">{{$message}}</p>
                        @enderror
                    </div>
